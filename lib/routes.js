module.exports = function(app, panicPubnub, panicQueue){
  ...
  app.all('*', function(req, res, next){
    ...
    */
    ...
    var newPanics = panicQueue.recent.all(req.session.lastSynced);
  ...
  app.get('/action/backup', function(req, res){
    ...
    req.session.panicLog = cleanPanicLog(req.session.panicLog, parentMsgDate);
  });

  app.get('/action/standdown', function(req, res){
    console.log('Standing down for ',req.query['date']);
    var parentMsgDate = Number(parentMsgDate);
    /*
    (req.query['date'] == "")
    ? null : Number(parentMsgDate);
    */
  ...
  function cleanPanicLog(log, msgTime){
    var filtered = log.filter(function(msg){
      if(msg.date == msgTime){
        return false;
      } else {
        return true;
  ...
  app.get('/action/dismiss', function(req, res){
    ...
    if(date2 == false){    // including empty string
      ...
      req.session.panicLog = cleanPanicLog(req.session.panicLog, date2);
      res.redirect('back');
    }
  });
